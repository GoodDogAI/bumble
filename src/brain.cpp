    ...
    if (image_ptr) {
        ...
        for(int i=0; i < image_ptr->height; i++) {
            for(int j=0; j < image_ptr->width; j++) {
                hostInputBuffer[0 * image_ptr->height * image_ptr->width + i * image_ptr->width + j] = image_ptr->data[i * image_ptr->step + j + 0] / 255.0;
                hostInputBuffer[1 * image_ptr->height * image_ptr->width + i * image_ptr->width + j] = image_ptr->data[i * image_ptr->step + j + 1] / 255.0;
                hostInputBuffer[2 * image_ptr->height * image_ptr->width + i * image_ptr->width + j] = image_ptr->data[i * image_ptr->step + j + 2] / 255.0;
            }
        }
